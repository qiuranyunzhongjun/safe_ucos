...
#define  OS_TASK_TMR_PRIO 65533                         /*这个可能是添加安全功能自己加的*/
...
/*安全数据区的错误代码*/
#define OS_ERR_SAFE_NO_MEM            200u
#define OS_ERR_SAFE_INVALID_BLK_MIN   201u
#define OS_ERR_SAFE_INVALID_INERVAl   202u
#define OS_ERR_SAFE_SMALL_TOTAL_SIZE  203u
#define OS_ERR_SAFE_SMALL_BLOCK_COUNT 204u
#define OS_ERR_SAFE_INVALID_SIZE      205u
#define OS_ERR_SAFE_INVALID_PMEM      206u
#define OS_ERR_SAFE_TOO_LARGE_SIZE    207u
#define OS_ERR_SAFE_NO_SUIT_BLKS      208u
#define OS_ERR_SAFE_INVALID_PDATA     209u
#define OS_ERR_SAFE_INVALID_PBLK      210u
#define OS_ERR_SAFE_MEM_FULL          211u
#define OS_ERR_SAFE_CONFLICT_STRUCT   212u /*由于内存块存储冲突导致错误的识别了某一块内存块在链表中*/

#define OS_ERR_SAFE_INVALID_RULE      220u
#define OS_ERR_SAFE_RULE_SMALL_SIZE   221u
#define OS_ERR_SAFE_RULE_MORE_CON     222u
#define OS_ERR_SAFE_VAR_NOT_EXIST     223u
#define OS_ERR_SAFE_RULE_NOT_FIT      224u
#define OS_ERR_SAFE_RULE_LOCK_SMALL   225u

#define OS_ERR_SAFE_VAR_NAME_LONG     230u
#define OS_ERR_SAFE_VAR_INVALID_NAME  231u
#define OS_ERR_SAFE_VAR_INVALID_TYPE  232u
#define OS_ERR_SAFE_VAR_NAME_EXIST    233u
#define OS_ERR_SAFE_ARR_INVALID_NUM   234u
#define OS_ERR_SAFE_ARRAY_INDEX       235u
#define OS_ERR_SAFE_ARRAY_BOUND       236u
...
/*
*********************************************************************************************************
*                          MY SAFE MEMORY PARTITION DATA STRUCTURES 安全数据区 数据结构
*********************************************************************************************************
*/

#if (OS_SAFE_MEM_EN > 0u)

typedef struct os_safe_mem_block {                   /* 安全数据区的分块                                */
	void   *OSLastPhyMemBlk;                    /* 指向上一块物理位置相连的分块                 */
	void   *OSNextPhyMemBlk;                   /* 指向下一块物理位置相连的分块                   */
	//struct os_safe_mem_block  *OSLastListMemBlk;                   /* 指向上一块链表位置相连的分块                */
	//struct os_safe_mem_block  *OSNextListMemBlk;                   /*指向下一块链表位置相连的分块                  */
} OS_SAFE_MEM_BLOCK;
typedef struct os_safe_mem_list_block {                   /* 链表内存块链接数据结构        */
	void   *OSNextListMemBlk;                   /* 指向下一块链表相连的分块                   */
	void   *OSLastListMemBlk;                    /* 指向上一块链表相连的分块                 */
} OS_SAFE_MEM_LIST_BLOCK;

typedef struct os_safe_mem_used_block {                   /* 用户申请后的内存块数据结构        */
	INT8U   prio;                   /* 使用该内存块的任务优先级，也是唯一标识符。任务更改优先级之后也应该修改这个数字                  */
#if OS_TASK_CREATE_EXT_EN > 0u
	INT32U   CreateTime;             /* 使用该内存块的任务创建时间，为了防止重复利用同一个优先级创建任务                  */
#endif
} OS_SAFE_MEM_USED_BLOCK;


typedef struct os_safe_part_mem {                   /* MEMORY CONTROL BLOCK 用来记录每一种大小的内存块的链表信息 */
	void  *OSSafeMemPartFreeList;                /* Pointer to list of free memory blocks                     */
	INT32U  OSafeMemPartBlkSize;                 /* Size (in bytes) of each block of memory                   */
	INT32U  OSSafeMemPartNFree;                   /* Number of memory blocks remaining in this partition       */
#if (OS_SAFE_MEM_MERGE_EN == 0u) && (OS_TASK_CREATE_EXT_EN > 0u)
	INT32U  OSafeMemPartUsedCount;                    /* 统计域                                     */
#endif
} OS_SAFE_PART_MEM;

typedef struct os_safe_mem {                   /* SAFE MEMORY CONTROL BLOCK                                      */
	void     *OSSafeMemAddr;                    /* Pointer to beginning of safe memory                  */
	OS_SAFE_PART_MEM  SafeMemParts[OS_SAFE_MEM_BLOCK_COUNT];         /* 存放各种大小的内存块链表的数组 */
	INT32U  OSSafeMemTotalSize;                   /* 总空间                  */
	INT32U  OSSafeMemFreeSize;                   /* 剩余空间      */
	INT32U  OSSafeMemPartMaxIndex;                     /*安全区最大支持的内存块大小所在的下标*/
} OS_SAFE_MEM;

typedef struct os_safe_mem_list {                   /* 安全数据区链表                                      */
	void   *OSVarAddr;                    /* 指向变量存放的地址                  */
	void   *OSMemFreeList;                /* Pointer to list of free memory blocks                     */
	INT32U  OSMemBlkSize;                 /* Size (in bytes) of each block of memory                   */
	INT32U  OSMemNBlks;                   /* Total number of blocks in this partition                  */
	INT32U  OSMemNFree;                   /* Number of memory blocks remaining in this partition       */
...
typedef struct part_data {
	INT32U  PartBlkSize;                 /* Size (in bytes) of each block of memory                   */
	INT32U  PartNFree;                   /* Number of memory blocks remaining in this partition       */
#if OS_TASK_CREATE_EXT_EN > 0u
	INT32U  PartUsedCount;                    /* 统计域                                     */
#endif
}PART_DATA;
typedef struct os_safe_mem_data {
	void     *SafeMemAddr;                    /* Pointer to beginning of safe memory                  */
	PART_DATA  Parts[OS_SAFE_MEM_BLOCK_COUNT];         /* 存放各种大小的内存块链表的数组 */
	INT32U  TotalSize;                   /* 总空间                  */
	INT32U  FreeSize;                   /* 剩余空间      */
	INT32U   PartCount;
} OS_SAFE_MEM_DATA;

typedef struct op_priority
{
	char* op; /*规则字符串中运算符*/
	int pri; /*规则字符串中运算符的优先级*/
}OS_RULE_OP_PRI;

typedef struct os_safe_rule {                   /* 规则变量控制块 CONTROL BLOCK                                      */        
	void   *rule;
	struct os_safe_rule* next;         /*指向下一个规则变量*/
} OS_SAFE_RULE;

typedef struct os_safe_variable {                   /* 安全变量控制块 CONTROL BLOCK                                      */
	char name[32];                    /*安全变量的名字，暂时只最多支持31个字符的变量名字*/
	OS_SAFE_RULE   *OSRuleList;                /* Pointer to list of 指向规则列表的指针                     */
	INT32U  OSSafeVarType;                 /* 安全变量的类型标识，前6位数字用于锁操作。前6位为0说明无需加锁，第7位1为加锁0为未加锁*/
	OS_PRIO OSEventTbl[OS_EVENT_TBL_SIZE];  /* 等待安全变量解锁的任务列表                */
	OS_PRIO OSEventGrp;
	struct os_safe_variable* next;         /*指向下一个安全变量*/
} OS_SAFE_VAR;

typedef union os_safe_var_data
{
	short short_value;
	int  int_value;
	long long_value;
...
OS_EXT  OS_SAFE_MEM      *OSSafeMem;                   /* 安全数据区首地址  */
#if OS_SAFE_MEM_MERGE_EN == 1u
extern  INT32U  const    OSSafeBlockNum[(OS_SAFE_MEM_BLOCK_MAX - OS_SAFE_MEM_BLOCK_MIN) / OS_SAFE_MEM_BLOCK_INERVAl + 1];                   /* 安全数据区递增内存区每种大小的内存块的保留个数  */
#endif
OS_EXT  void             *OSSafeVars[64];            /*存放每个优先级的任务的安全变量指针*/
OS_EXT  INT32U            OSSafeVarSizes[64];            /*存放每个优先级的任务需要等待分配的安全变量大小*/
OS_EXT  OS_PRIO           OSSafeMemTbl[OS_EVENT_TBL_SIZE]; /* 安全内存等待任务列表 */
OS_EXT  OS_PRIO			  OSSafeMemGrp; /* 安全内存等待任务所在的组 */
OS_EXT  char              log_info[1024];/*日志记录相关的字符串*/
...
void          *OSSafeVarMemPend       (INT8U  size, 
									   INT32U *timeout, 
									   INT8U   *perr);

INT8U          OSSafeVarMemPut        (void *pblk);

INT8U          OSSafeVarMemQuery      (OS_SAFE_MEM_DATA  *p_mem_data);
...
*********************************************************************************************************
*/
INT8U          OSSafeVarCreate       (char * name,
							          char * type,
									  ...);

INT8U          OSSafeArrayCreate     (char * name, 
									  char * type, 
									  int num);

INT8U		  OSSafeVarCreateWait    (char * name, 
									  char * type, 
									  INT16U timeout,
									  ...);

INT8U		  OSSafeArrayCreateWait  (char * name, 
									  char * type, 
									  int num, 
									  INT16U timeout);
...
INT8U		   OSSafeVarNoCheckSet   (char * name, 
									  void* value);

OS_SAFE_VAR_DATA  *OSSafeVarGet      (char * name, 
									  INT8U   *perr);

INT8U		   OSSafeArraySet		 (char * name, 
									  int  index, 
									  ...);

INT8U		   OSSafeArrayNoCheckSet  (char * name, 
									  int  index, 
									  ...);

INT8U		  OSSafeArrayNoCheckNCopy(char * name, 
									  void* value, 
									  int  n);

OS_SAFE_VAR_DATA  *OSSafeArrayGet    (char * name, 
									  int index, 
									  INT8U   *perr);
...
INT8U		   OSSafeRuleInsertWait  (char * rule, 
									 INT32U timeout);
